...
| Dung and IBIS re-bound to their new source | provider ids `rdam.dung/exhaustive-subset-v1` and `rdam.ibis/gibis-grammar-v1`; source digests recomputed from the relocated modules and reported as `provenance.source_revision` | `Machine([DungProvider(), IbisProvider()]).capabilities()` — both `available` |
