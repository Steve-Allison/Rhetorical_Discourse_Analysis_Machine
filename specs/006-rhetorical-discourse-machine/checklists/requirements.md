...
| **FR-027** — existing artifacts assessed, not presumed complete | T007 | **MET, and it bit.** The T007 audit found the then-current promotion flow incomplete. The promotion system was removed entirely on 2026-09-02 by owner ruling, so the finding is now moot; the requirement's point — never presume an artifact complete because it exists — stands. |
...
| **SC-006** — provider provenance completeness | 011, 012, 009 | **Met.** Every available provider declares package, version, source digest, model identity where it has one, and licence; the criterion was rewritten on 2026-09-02 when the promotion-evidence system was removed. |
