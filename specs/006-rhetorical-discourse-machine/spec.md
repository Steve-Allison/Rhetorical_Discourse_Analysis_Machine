...
- **SC-009**: Each of the ten planned follow-on capabilities has its own decision-closed Spec Kit feature and passes cross-artifact consistency analysis before implementation begins.
...
- The follow-on feature family is: aggregate analysis contract, workbench promotion system, RST provider adapter, Dung provider, IBIS provider, SDRT provider, Toulmin provider, Walton provider, PDTB provider, and cross-provider orchestration.
