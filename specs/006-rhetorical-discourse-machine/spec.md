...
- Estate division of analytical labour (owner ruling 2026-09-01): this machine is the sole estate authority for technique-native discourse and argumentation structures — RST/eRST trees, Dung argumentation frameworks, IBIS structures, SDRT graphs, and any future promoted technique result, at every capability tier including heavy LLM-assisted analysis if a candidate ever passes promotion. The AI_Skills `concept-extract` skill is the deliberately bounded "Lite" tier: portable, LLM-only, vocabulary-typed span annotations against the registered Central_Configs sets, and never a producer of technique-native structures. Content_Structuring_Machine is the machine's primary named consumer: it orchestrates heavy-duty content structuring, consumes analyses only through the supported public contracts (`isanlp_rst` today; the aggregate and native contracts once the aggregate-contract feature lands), and owns no analysis semantics of its own — its RST bridge (its spec 003) is expected to converge to a thin client over the public contract. The aggregate-contract and RST-provider-adapter features take Content_Structuring_Machine's consumption needs as named design input.
