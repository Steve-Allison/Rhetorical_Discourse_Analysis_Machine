...
formally correct and receipt-backed eRST; complete static/runtime quality; and fail-closed research,
artifact, and release promotion. Format work centralizes leaf/span projection and DocLang eligibility.
eRST work replaces heuristic/gold-dependent candidate generation and DAG decoding with a single
signal-licensed formal pipeline, official partitions/scoring, Pydantic evidence boundaries, and
safetensors bundles. Research work first reproduces the published ELECTRA baseline, then evaluates
all mandatory systems under one immutable protocol. Release work validates the exact candidate on a
fresh environment and publishes no checkpoint or SOTA claim unless all gates pass.
...
model repository only for a promoted checkpoint; private local corpus/experiment workspace excluded
from Git and package archives

**Testing**: pytest through locked Pixi, Pyright full tree, Ruff, markdownlint-cli2, official eRST
scorer, paired document bootstrap, pip-audit, secret scan, build/archive inspection, CPU/MPS smokes
...
**Performance Goals**: No candidate truncation; longest test document completes without OOM; promoted
model <=24 GB peak RSS; MPS p95 <=2x reproduced ELECTRA; tied systems within 0.005 Full choose smaller
and faster
...
entry points, five primary parser variants, complete licensed GUM eRST candidate space, nine mandatory
research systems
...
|---|---|---|
| Evidence before claims | Immutable report/Graphify before-state and finding matrix | Every task ends in checkable evidence; promotion is fail-closed |
...
| Full scope and no false green | All original/new defects enumerated | No exclusion, warning filter, truncation, skipped mandatory system, or hidden failed gate |
...
├── eval/                        # official eRST scorer adapter + statistics
...
1. Add Pydantic corpus, split, protocol, run, checkpoint, and promotion boundaries.
...
6. Preserve raw relation labels plus ontology projection; integrate official scorer metrics.
...
### Phase 4 — Reproduce the published baseline
...
1. Materialize GUM V12.1.0 official splits and the official scorer with immutable hashes.
2. Reproduce signal-marked ELECTRA serialization and five seeds in gold/gold and predicted/predicted.
3. Persist checkpoints, manifests, predictions, scorer output, runtime profile, and variance.
4. Stop and diagnose if any gold/gold metric misses ±0.02.

**Success criterion**: A clean checkout reproduces the hashed baseline evidence and all three metric
bounds. Architecture selection remains blocked until this is true.
...
1. Run the nine mandatory systems using identical inputs and screening seeds.
...
5. Evaluate all promotion thresholds and emit `promote` or `no-promotion`.

**Success criterion**: Every mandatory system has a complete success/incompatibility receipt; any
promotion satisfies every threshold. Otherwise 4.0.0 carries no canonical/SOTA claim.
...
4. Update every closure row, regenerate Graphify, and record artifact hashes.
...
archive/install evidence, and immutable private model revision only if promotion passed.
...
No constitutional violation. The nine-model experiment matrix is required by the approved scope, not
an abstraction or scaling choice; private Hugging Face storage is the minimal reproducible channel
for a promoted bundle.
