...
- [X] T129 [US4] After T145 invalidates the pre-convergence evidence, rerun the Feature 004 focused tests plus Ruff and Pyright through Pixi and write canonical results through the T123 evidence contract to `specs/004-production-api-contract/evidence/pre-release-quality.json` (FR-036-FR-038)
- [X] T130 [US4] After T145 invalidates the pre-convergence evidence, rerun the one-warm-up/five-run preparation performance gate and write canonical per-run results through the T123 evidence contract to `specs/004-production-api-contract/evidence/performance.json` (SC-014)
- [X] T131 [US4] After T145 invalidates the pre-convergence evidence, audit the final source diff, including T121-T127, for consumer-specific fields, restored format-specific APIs, research/offline leakage, forbidden scientific internals, model architecture changes, inference-mathematics changes, unexplained backend evidence loss, fabricated decisions, runtime-identity contradictions, archived capability claims, and independent CLI/local-HTTP semantics, then revalidate that the dated comparison in `specs/004-production-api-contract/research.md` covers every FR-045 practice with zero unclassified gaps; record both dispositions in `specs/004-production-api-contract/evidence/scope-audit.md` (FR-025, FR-029-FR-030, FR-033-FR-034, FR-045-FR-046, FR-064-FR-073, SC-015, SC-018, SC-024-SC-028)
- [X] T132 [US4] After T129-T131 and T145, run the complete source-only lint, Pyright, Markdown, pytest, Feature 004 conformance, canonical parser-result, component-byte identity, capability truth, CLI/local-HTTP parity, decision-evidence loss, determinism, and performance gates, reconcile their current results, and persist canonical aggregate results and evidence digests in `specs/004-production-api-contract/evidence/source-release-gates.json`, containing no artifact or clean-install claims (FR-036-FR-041, FR-047-FR-073, SC-006-SC-009, SC-014, SC-017-SC-028)
- [ ] T133 [US4] Create the clean source release commit containing the final `isanlp_rst/`, `tools/`, `tests/`, `docs/`, `pyproject.toml`, `pixi.lock`, and `specs/004-production-api-contract/` source candidate after T132 passes and after the convergence prerequisites declared below are complete (FR-043-FR-044)
- [ ] T134 [US4] Generate canonical `specs/004-production-api-contract/evidence/source-release.json` after the converged T133 exists, recording the exact source commit, tree, archive, and commit-derived `SOURCE_DATE_EPOCH` identities without claiming its own future commit identity (FR-043-FR-044)
- [ ] T135 [US4] Build wheel and sdist twice from the named converged T133 source commit and select only byte-identical `dist/5.0.0/isanlp_rst-5.0.0-py3-none-any.whl` and `dist/5.0.0/isanlp_rst-5.0.0.tar.gz` artifacts (FR-042-FR-044)
...
- **Convergence release prerequisite**: Before Feature 004 resumes source or
  artifact selection, complete T149 and T150, then use T144 to verify that the
  selected immutable model release establishes the exact bytes it loads and
  executes the canonical typed parser-analysis operation. T145 invalidates and
  regenerates source-gate evidence through T129-T132; T133-T135 then rebuild
  the release before T136-T142. Model training, scientific evaluation, and
  model promotion are outside Feature 004 and must not be inferred from task
  boxes, import paths, or a release name.
...
- [X] T143 SUPERSEDED Record that ModernBERT training, scientific evaluation, quality thresholds, and model promotion are outside Feature 004; no Feature 004 implementation or completion claim may satisfy or prescribe that work per Constitution III and V and FR-029-FR-030 (contradicts; superseded by T151-T153)
- [X] T144 [US4] Validate Feature 004 consumption of one immutable `isanlp_rst.parser/modernbert-v1` release: inventory every tokenizer/configuration/weight/relation byte, prove strict exact-byte loading and canonical typed parser-analysis execution, reject any identity or executability contradiction through the public contract, and rerun installed capability/conformance acceptance before T145 or T136 per FR-069, FR-071, SC-026, and SC-027 (missing)
- [X] T145 Invalidate and remove every completion claim attached to the pre-convergence T133-T135 candidate, complete T149-T150 and T144, and rerun T129-T132 so the final scope and source-gate evidence identifies the exact converged source before T133 resumes per FR-043-FR-044 and SC-016 (partial)

## Phase 11: Post-Upgrade Convergence

- [X] T146 SUPERSEDED Record that model-quality evaluation and release-threshold enforcement are outside Feature 004; its public contract reports only evidence it genuinely produces or receives at runtime (FR-066, FR-071, SC-027; superseded by T151-T153)
- [X] T147 SUPERSEDED Record that training, benchmark, ledger, and evaluator-parity implementation are outside Feature 004; its consumer boundary must neither fabricate nor infer those values (FR-066; superseded by T151-T153)
- [X] T148 SUPERSEDED Record that model-promotion policy and candidate evidence are outside Feature 004; T144 verifies only the exact model component identity and executable public runtime contract it consumes (FR-066, FR-069, SC-026; superseded by T151-T153)
- [X] T149 Update the canonical `production-clean-install` task in `pyproject.toml` and its release evidence path to require `--full`, the explicit promoted ModernBERT release ID, the governed model store, network-disabled real inference, and retained core/formats results; reject a preparation-only run as release certification before T136 (FR-036, FR-069, FR-071, SC-010, SC-013, SC-026-SC-027)
- [X] T150 Extend the production-boundary ownership contract and tests to classify the governed ModernBERT candidate and immutable in-tree release files without weakening the prohibition on model weights inside the Python wheel/sdist, then require both source and artifact boundary scans to pass before T145-T136 (FR-029-FR-031, FR-069, SC-010, SC-015-SC-016, SC-026)

## Phase 12: Cross-Feature Authority Convergence

- [X] T151 CRITICAL Remove the downstream-driven Feature 005 qualification condition from `specs/004-production-api-contract/spec.md` and `plan.md`; constrain Feature 004 to its provider-owned API, runtime-byte identity, capability, distribution, and release-boundary obligations per Constitution III and V and FR-029-FR-030 (contradicts)
- [X] T152 CRITICAL Reconcile `specs/004-production-api-contract/tasks.md` so T143 and T146-T148 are explicitly outside Feature 004, T144 validates Feature 004 consumption of the actual immutable runtime release, and unsupported completion claims on T129-T135 are reopened before release certification resumes per Constitution I and IV and FR-043-FR-044 (contradicts)
- [X] T153 Add an explicit Feature 004 convergence dependency sequence requiring T149-T150, T144, T145, T129-T132, T133-T135, and only then T136-T142; preserve every API and release-boundary obligation without creating a Feature 005 dependency per plan: Phase F, FR-036, FR-042-FR-044, FR-069, FR-071, and SC-010-SC-016 (partial)
