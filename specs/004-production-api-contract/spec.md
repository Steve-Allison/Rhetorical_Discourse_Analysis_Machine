...
- **FR-073**: If the local HTTP adapter remains a supported installed capability, it MUST be loopback-only and use the same typed request, success, failure, serialization, component-identity, and evidence contracts as the Python and command-line APIs; it MUST NOT expose raw exception strings, return count-only analysis as a complete result, become a separate semantic authority, or imply a hosted or multi-user service guarantee.
