...
Feature 018's original implementation preceded completion of Feature 017. The
owner subsequently approved Feature 017's canonical ingest move, clean API break,
declared provider parallel safety, and separate analytical/artifact identities.
Those decisions remain authoritative: this final 018 pass does not restore
retired interfaces, blanket locks, or pre-017 serialized envelopes. The requirements
below reflect that reconciliation; source preparation remains owned by Feature 017.
...
2. **Given** a current native result without `source_revision`, **When** it loads, **Then** its canonical bytes round-trip exactly; available provider declarations still require a revision.
...
3. **Given** a shared provider instance declaring `serialized`, **When** separate machines call it concurrently, **Then** its calls are serialized; providers declaring `concurrent` may overlap.
...
- **FR-004**: Aggregate/native contract identifiers and version MUST remain unchanged by this pass; current canonical bytes MUST round-trip exactly. Pre-017 envelope compatibility is not required by the owner's clean-break ruling.
...
- **FR-008**: Provider `source_identity()` functions MUST delegate to the shared identity helper rather than duplicate its implementation.
...
- **FR-018**: Calls to one provider instance declaring `serialized` MUST serialize across concurrent machine use; `concurrent` providers may overlap, as required by Feature 017 FR-040.
...
- **FR-021**: Dirty, missing, empty, and unknown source revisions MUST bypass caching, including revisions decorated with an instructions digest.
...
- **FR-027**: This feature MUST NOT change `specs/017-universal-source-pipeline/` or alter its completed `rdam/ingest/` ownership, Docling, DocLang, Markdown, harvest, preparation, or source-pipeline behavior.

## Success Criteria

- **SC-001**: Deep-mutation tests cannot change a validated record or its digest.
- **SC-002**: The no-revision fixture and current `1.0.0` serialization tests round-trip byte-identically under the post-017 contract.
