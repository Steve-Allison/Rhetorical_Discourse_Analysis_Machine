...
def _assert_tree_aligned(tree, original_text: str, path: str = "root") -> None:
    ...
    assert tree.text == expected, (
        f"{path}: tree.text={tree.text!r} != original_text[{tree.start}:{tree.end}]={expected!r}"
    )
...
def _run_family(version: str, device: str = "cpu", dtype=None, **extras) -> None:
    ...
    parser = Parser(
        hf_model_name="tchewik/isanlp_rst_v3", hf_model_version=version, device=device, dtype=dtype, **extras
    )
    ...
    _check("parse_from_edus round-trip", lambda: _check_from_edus(parser, list(SAMPLE_EDUS)))
    _check("parse_from_edus single EDU", lambda: _check_from_edus(parser, ["Just one EDU here."]))
    _check("parse_from_edus empty raises", lambda: _expect_raises(lambda: parser.from_edus([]), ValueError))
    _check(
        "parse_from_edus empty-string EDU raises",
        lambda: _expect_raises(lambda: parser.from_edus(["ok", ""]), ValueError),
    )
    _check(
        "parse_from_edus string-not-list raises",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        _check("detect_family_from_model_dir DMRST", lambda: test_facade_detect_dmrst_local(tmp_path))
        _check("detect_family_from_model_dir UniRST (pickle)", lambda: test_facade_detect_unirst_local(tmp_path))
        _check(
            "detect_family_from_model_dir UniRST (config.corpora)",
    ...
    if unirst_parser_for_offset_check is not None:
        print("\n=== _guess_token_offsets raise-on-miss (UniRST tokens path) ===", flush=True)
        _check(
            "parse_rst with mismatched tokens raises ValueError",
            lambda: test_unirst_guess_offsets_raises(unirst_parser_for_offset_check.predictor),
        )
