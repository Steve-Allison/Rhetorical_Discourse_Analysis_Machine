"""The RST provider: ``isanlp_rst`` presented to the machine through its own declaration.
...
resolves. Failures are ``isanlp_rst``'s typed failures mapped one-to-one — same code, same
...
from isanlp_rst.erst.checkpoint import resolve_default_erst_checkpoint
...
class RstProvider:
    """One configured ``isanlp_rst`` release, declared to the machine (006 data-model §Provider)."""
    ...
    def provider_id(self) -> str:
        return f"isanlp_rst/{self._release_id}"
    ...
    def declaration(self) -> ProviderDeclaration:
        ...
        return ProviderDeclaration(
            ...
            provenance=ProviderProvenance(
                package="isanlp_rst",
                version=isanlp_rst.__version__,
