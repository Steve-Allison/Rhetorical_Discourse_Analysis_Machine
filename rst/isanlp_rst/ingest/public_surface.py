...
from isanlp_rst.ingest.contracts.base import StrictContractModel
...
def load_public_surface() -> PublicSurfaceInventory:
    ...
    payload = resources.files("isanlp_rst.ingest").joinpath(_RESOURCE).read_bytes()
...
def _schema_exists(schema_id: str) -> bool:
    ...
    resource = resources.files("isanlp_rst.ingest").joinpath("schemas", name)
