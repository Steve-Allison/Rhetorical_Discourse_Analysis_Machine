"""
Offline DMRST multiple-run experiment orchestration.
...
class MultipleRunnerGeneral:
    ...
    def train(self) -> None:
        ...
        for run in self._get_variants():
            ...
            p = subprocess.Popen(
                ["python", "dmrst_parser/trainer.py", "configs/general_config.jsonnet", json.dumps(general_parameters)],
                stdout=sys.stdout,
                stderr=sys.stderr,
            )
            p.wait()
    ...
    def evaluate(self) -> None:
        ...
        Path(f"{self.lang}_{self.corpus}_{self.model_type}_all_res.json").write_text(json.dumps(results))
