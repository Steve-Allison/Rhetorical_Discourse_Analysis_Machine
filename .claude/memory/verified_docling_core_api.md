...
**Default content-layer filter:** `DEFAULT_CONTENT_LAYERS = {ContentLayer.BODY}` at `document.py:1291`. Page headers, footers, slide masters, and other furniture-layer content are excluded automatically. To include furniture, pass `included_content_layers={ContentLayer.BODY, ContentLayer.FURNITURE}`.
