...
from rdam.ingest.contracts.source import SourceArtifact, SourceForm, SourceSummary
from rdam.ingest.identity import semantic_sha256
from rdam.ingest.capabilities import describe_capabilities
from rdam.ingest.cache import ProductionIngestCache, cache_entry_identity
from rdam.ingest.policy import DEFAULT_PLANNING_POLICY, DEFAULT_PREPARATION_POLICY
from rdam.ingest.prepare import (
    PreparationValidationError,
    SourceClassificationError,
    prepare_source,
)
from rdam.ingest.subdivision import AnalysisPlanningError
...
class ProductionIngestor:
    ...
    def analyse(
        ...
        *,
        ...
        if preparation is not None:
            if preparation.semantic.source != source.summary():
                raise ValueError("preparation belongs to a different source")
            return self.analyse_prepared(preparation, analysis_policy=analysis_policy, cache_directory=cache_directory)
        parser = self.parser
        try:
            capacity = declared_capacity(parser.analysis_capacity) if parser is not None else None
        except Exception as exc:
            failure = ProductionFailure(
                failed_stage=LifecycleStage.PLANNING,
                category=FailureCategory.PROVIDER_UNAVAILABLE,
    ...
    def analyse_prepared(
        ...
        *,
        ...
        """Analyse a validated preparation without reading or inventorying its source again."""
        ...
        if plan.status is AnalysisPlanStatus.SUBDIVIDED:
            ...
            from rdam.ingest.recombination import recombine_parser_results
