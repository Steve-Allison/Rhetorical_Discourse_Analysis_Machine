...
from rdam.ingest import _harvest
...
from rdam.ingest.policy import (
...
def prepare_source(
    ...
    *,
    ...
    capacity: AnalysisCapacity | None = None,
    ...
    semantic = assemble_preparation(
        artifact.summary(), inventory, source_contract,
        policy=policy, planning_policy=planning_policy, capacity=capacity,
        empty_submitted_content=_submitted_content_is_empty(artifact),
    )
    outcome = PreparationOutcome(
        semantic=semantic,
        execution=PreparationExecutionEvidence(
            execution_id=execution_id or str(uuid4()),
            adapters=(AdapterExecutionIdentity(
                distribution=source_contract.upstream_format or "isanlp_rst",
                version=source_contract.upstream_version or version("rdam"),
            ),),
...
def assemble_preparation(
    ...
    *,
    ...
    """Pure assembly of an already inventoried and dispositioned source."""
    ...
    for item_index, item in enumerate(primary):
        if item_index:
            separator = " " if source.source_form.value == "edus" else "\n\n"
...
def _translate_item(
    ...
    return ContentInventoryItem(
        ...
        speaker=resolve_speaker(legacy.text or "", legacy.attributes) if classification is ContentClass.TURN else None,
...
def _table_representation(
    ...
    for child in table_cells:
        anchor = next(
            (candidate for candidate in child.native_anchors if candidate.kind is LegacyAnchorKind.TABLE_COORDINATE),
            None,
        )
        if anchor is None:
            # DocLang also inventories the XML wrappers and row markers. They
            # are source items, not additional cells with guessed coordinates.
            if current_cell is not None and child.text is not None:
                linked[current_cell].append(child.item_id)
            continue
        values = _selector_values(anchor.selector)
        if "row" not in values or "column" not in values:
            raise ValueError("table cell requires source-declared row and column")
        row = _leading_integer(values["row"])
        column = _leading_integer(values["column"])
        if (row, column) in coordinates:
            raise ValueError("multiple table cells declare the same source coordinate")
        coordinates[row, column] = (child, values.get("token", "fcel"))
        current_cell = child.item_id
        linked[current_cell] = list(child.child_ids)
    ...
    for (row, column), (child, token) in sorted(coordinates.items()):
        ...
        cells.append(
            TableCell(
                ...
                text=child.text,
                header=(
                    token in {"ched", "rhed", "corn", "srow"}
                    or attributes.get("column_header") == "true"
                    or attributes.get("row_header") == "true"
                ),
                linked_item_ids=tuple(dict.fromkeys(linked[child.item_id])),
            )
        )
