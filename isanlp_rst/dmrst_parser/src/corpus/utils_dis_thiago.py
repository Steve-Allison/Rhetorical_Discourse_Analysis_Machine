...
def createnode(node: data.SpanNode, content: list[object]) -> data.SpanNode:
    """
...
def buildTreeThiago(
    text: str,
) -> tuple[data.SpanNode, list[int], list[data.SpanNode], dict[int, str]]:
    """
...
def findChild(beg: object, end: object, misplaced_children: list[data.SpanNode]) -> data.SpanNode | None:
    for c in misplaced_children:
...
def getDisFiles(tbpath: str | Path) -> tuple[list[Path], list[Path]]:
    root = Path(tbpath)
    disFiles = [path for path in root.iterdir() if path.is_file() and path.name.endswith(".dis")]
    eduFiles = [path for path in root.iterdir() if path.is_file() and path.name.endswith(".edus")]
...
def readEduDoc(fedu: str | Path, doc: Any) -> Any:
    ...
    if not edu_path.is_file():
        raise FileNotFoundError(f"File doesn't exist: {edu_path}")
    # EDU ids start at 1
    gidx, eidx, tokendict, edudict = 0, 1, {}, {}
    with edu_path.open("r") as fin:
        for line in fin:
            line = line.strip()
            if len(line) == 0:
                continue
