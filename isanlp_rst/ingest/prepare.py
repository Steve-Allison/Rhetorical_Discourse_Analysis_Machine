"""Complete source inventory and reversible canonical preparation."""
...
from isanlp_rst.contracts.document import RstDocument, SourceReference
...
def prepare_source(
    ...
    *,
    ...
    """Prepare one source and return complete semantic evidence."""

    inventory, contract = inventory_source(artifact)
    _verify_inventory(inventory)
    dispositions, _duplicates = apply_policy(inventory, policy)
    disposition_by_id = {item.item_id: item for item in dispositions}
    primary_items = tuple(
        item
        for item in inventory
        if disposition_by_id[item.item_id].kind is DispositionKind.PRIMARY and item.text is not None
    )
    ...
    for index, item in enumerate(primary_items):
        if index:
            separator = " " if artifact.source_form is SourceForm.EDUS else "\n\n"
            segments.append(
                PreparedSegment(
                    segment_id=f"separator:{index}",
        ...
        text = item.text or ""
        segment_id = f"source:{item.item_id}"
        segments.append(
            PreparedSegment(
                ...
                source_range=PreparedRange(start=0, end=len(text)),
                original_text=text,
                native_anchors=item.native_anchors,
                structure_path=_structure_path(item, inventory),
            )
        )
        cursor += len(text)

    prepared_text = "".join(segment.text for segment in segments)
    if artifact.source_form is SourceForm.EDUS:
        document = RstDocument.from_edus(
            artifact.edus or (),
            document_id=artifact.source_id,
            source=SourceReference(uri=artifact.original_source, locator=artifact.source_name, mime_type=artifact.media_type),
        )
        if document.text != prepared_text:
            raise ValueError("presegmented EDU preparation diverged from exact EDU contract")
    else:
    ...
    segment_by_item = {
        segment.source_item_id: segment for segment in segments if segment.source_item_id is not None
    }
    structure = tuple(
        StructureNode(
            ...
            kind=_structure_kind(item.content_class),
    ...
    prepared = PreparedRstDocument(
        ...
        structure=structure,
        primary_item_ids=tuple(item.item_id for item in primary_items),
        side_channel_item_ids=tuple(
            item.item_id
            for item in inventory
            if disposition_by_id[item.item_id].kind is not DispositionKind.PRIMARY
        ),
    )
    enriched_dispositions = tuple(
        disposition.model_copy(
            update={
                "prepared_segment_ids": (
                    (segment_by_item[disposition.item_id].segment_id,)
                    if disposition.item_id in segment_by_item
                    else ()
                )
            }
        )
    ...
    return prepared, inventory, enriched_dispositions, contract
...
__all__ = ["inventory_source", "prepare_source"]
