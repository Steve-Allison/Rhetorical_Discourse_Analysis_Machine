...
def test_flatten_tree_ids_are_unique_across_relations_and_edus() -> None:
    ...
    root = FakeUnit(start=0, end=10, left=leaf_a, right=leaf_b, relation="elaboration", nuclearity="NS")
...
def test_flatten_tree_left_right_ids_resolve_into_id_set() -> None:
    ...
    inner = FakeUnit(start=0, end=10, left=leaf_a, right=leaf_b, relation="elaboration", nuclearity="NS")
    root = FakeUnit(start=0, end=15, left=inner, right=leaf_c, relation="background", nuclearity="SN")
...
def test_flatten_tree_root_id_is_zero() -> None:
    ...
    leaf_a = FakeUnit(start=0, end=5)
    leaf_b = FakeUnit(start=5, end=10)
    root = FakeUnit(start=0, end=10, left=leaf_a, right=leaf_b, relation="x", nuclearity="NS")
    spans = (_span("#/blocks/0", 0, 5), _span("#/blocks/1", 5, 10))
...
def test_ns_routes_left_to_nucleus_right_to_satellite() -> None:
    ...
    root = FakeUnit(start=0, end=10, left=leaf_l, right=leaf_r, relation="elab", nuclearity="NS")
    spans = (_span("#/blocks/0", 0, 5), _span("#/blocks/1", 5, 10))
    relations, _ = flatten_tree(root, spans, ())
...
def test_sn_routes_right_to_nucleus() -> None:
    leaf_l = FakeUnit(start=0, end=5)
    leaf_r = FakeUnit(start=5, end=10)
    root = FakeUnit(start=0, end=10, left=leaf_l, right=leaf_r, relation="cause", nuclearity="SN")
    spans = (_span("#/blocks/0", 0, 5), _span("#/blocks/1", 5, 10))
    relations, _ = flatten_tree(root, spans, ())
...
def test_nn_routes_both_to_nucleus_empty_satellite() -> None:
    ...
    leaf_l = FakeUnit(start=0, end=5)
    leaf_r = FakeUnit(start=5, end=10)
    root = FakeUnit(start=0, end=10, left=leaf_l, right=leaf_r, relation="joint", nuclearity="NN")
    spans = (_span("#/blocks/0", 0, 5), _span("#/blocks/1", 5, 10))
    relations, _ = flatten_tree(root, spans, ())
...
def test_boundary_membership_lists_only_intersecting_boundaries() -> None:
    ...
    leaf_a = FakeUnit(start=0, end=5)
    leaf_b = FakeUnit(start=5, end=10)
    root = FakeUnit(start=0, end=10, left=leaf_a, right=leaf_b, relation="elab", nuclearity="NS")
    spans = (_span("#/blocks/0", 0, 5), _span("#/blocks/1", 5, 10))
    boundaries = (
        Boundary(id="section-0", kind="section", label="A", parent_block_ref=None, block_refs=("#/blocks/0",), level=1),
        Boundary(id="section-1", kind="section", label="B", parent_block_ref=None, block_refs=("#/blocks/1",), level=1),
    )
