...
def dmrst_gumrrg_cpu() -> Parser:
    return Parser(hf_model_name="tchewik/isanlp_rst_v3", hf_model_version="gumrrg", device="cpu")


@pytest.fixture(scope="session")
def dmrst_rstdt_cpu() -> Parser:
    return Parser(hf_model_name="tchewik/isanlp_rst_v3", hf_model_version="rstdt", device="cpu")


@pytest.fixture(scope="session")
def dmrst_rstreebank_cpu() -> Parser:
    """Russian-trained DMRST model."""
    return Parser(hf_model_name="tchewik/isanlp_rst_v3", hf_model_version="rstreebank", device="cpu")


@pytest.fixture(scope="session")
def unirst_eng_cpu() -> Parser:
    return Parser(
        hf_model_name="tchewik/isanlp_rst_v3", hf_model_version="unirst", device="cpu", relinventory="eng.erst.gum"
    )
...
def rrtrrg_cpu() -> Parser:
    ...
    return Parser(hf_model_name="tchewik/isanlp_rst_v3", hf_model_version="rrtrrg", device="cpu")
...
def test_dmrst_dtype_equivalence_on_mps(dmrst_gumrrg_cpu: Parser, text_name, text, dtype):
    ...
    mps_parser = Parser(hf_model_name="tchewik/isanlp_rst_v3", hf_model_version="gumrrg", device="mps", dtype=dtype)
    candidate = _topology(mps_parser(text)["rst"][0])
    assert candidate == baseline, f"DMRST gumrrg MPS {dtype} on {text_name} diverged from CPU fp32"
...
def test_unirst_russian_via_relinventory():
    ...
    parser = Parser(
        hf_model_name="tchewik/isanlp_rst_v3", hf_model_version="unirst", device="cpu", relinventory="rus.rst.rrt"
    )
...
def test_dtype_equivalence_rstdt(dmrst_rstdt_cpu: Parser, dtype):
    ...
    p = Parser(hf_model_name="tchewik/isanlp_rst_v3", hf_model_version="rstdt", device="mps", dtype=dtype)
...
def test_dtype_equivalence_rstreebank(dmrst_rstreebank_cpu: Parser, dtype):
    ...
    p = Parser(hf_model_name="tchewik/isanlp_rst_v3", hf_model_version="rstreebank", device="mps", dtype=dtype)
    assert _topology(p(RUSSIAN_TEXT)["rst"][0]) == baseline, f"rstreebank {dtype} diverged (Russian text)"
...
def test_dtype_equivalence_rrtrrg(rrtrrg_cpu: Parser, dtype):
    ...
    p = Parser(hf_model_name="tchewik/isanlp_rst_v3", hf_model_version="rrtrrg", device="mps", dtype=dtype)
