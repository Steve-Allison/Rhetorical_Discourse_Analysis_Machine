...
def test_docling_complete_current_traversal_reconciles_to_inventory() -> None:
    ...
    inventory, _ = inventory_source(
        SourceArtifact.from_path(PDF, source_form=SourceForm.DOCLING_JSON)
    )
...
def test_docling_picture_classification_producer_and_confidence_are_preserved() -> None:
    ...
    classified = [dict(item.attributes) for item in inventory if "picture_class" in dict(item.attributes)]
    assert classified
...
def test_docling_table_cells_keep_parentage_and_native_coordinates_but_never_default_to_prose() -> None:
    ...
    table_cells = [item for item in inventory if item.content_class is ContentClass.TABLE_CELL]
    assert table_cells
    assert all(item.parent_id in by_id for item in table_cells)
    assert all(
        any(anchor.kind is AnchorKind.TABLE_COORDINATE for anchor in item.native_anchors)
        for item in table_cells
    )
    prepared, _inventory, dispositions, _contract = prepare_source(artifact)
    primary_ids = set(prepared.primary_item_ids)
    assert all(item.item_id not in primary_ids for item in table_cells)
    assert all(
        disposition.kind.value != "primary"
        for disposition in dispositions
        if disposition.item_id in {item.item_id for item in table_cells}
    )
